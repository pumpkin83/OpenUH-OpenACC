...
#define _A(array, nx, ix, iy) (array[(ix) + nx * (iy)])
#define real double
	...
	real c0, real c1, real c2, real* w0, real* w1)
{
	...
	int im1m1, im1p1, ip1m1, ip1p1;
 	...
 	//#pragma omp for private(j,i,i00,im10,ip10,i0m1,i0p1,im1m1,im1p1,ip1m1,ip1p1)
	...
	for(j=1; j<ny-1; j++)
	{
		...
		*/
		   _A(w1, nx, i, j) = c0*_A(w0, nx, i, j) +
		   				  c1*(_A(w0, nx, i-1, j) + _A(w0, nx, i, j-1) + _A(w0, nx, i+1, j) + _A(w0, nx, i, j+1)) +
						  c2*(_A(w0, nx, i-1, j-1) + _A(w0, nx, i-1, j+1) + _A(w0, nx, i+1, j-1) + _A(w0, nx, i+1, j+1));
		}
	}
}
