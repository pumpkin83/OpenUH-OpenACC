...
int test_acc_kernels_async(FILE* logFile)
{
	int N = 1000;
	int A[N], B[N], C[N];
	int is_sync = -1;
	...
	/*test case 1: check the #pragma acc kernels async() with arguments*/
	#pragma acc kernels copyin(N, A[0:N], B[0:N]) copy(C[0:N]) async(1)
	  for(int i=0; i<N; i++)
	  {
	    C[i] = A[i] + B[i];
	  }
    ...
    /*test case 2: check #pragma acc kernels async without arguments, in this case,*/
    /*the implementation will use a value distinct from all explict async argu in the program*/
    #pragma acc kernels copyin(N, A[0:N], B[0:N]), copy(C[0:N]) async
	  for(int i=0; i<N; i++)
	  {
	    C[i] = A[i] + B[i];
	  }
	...
	for(int i=0; i<omp_get_num_threads(); i++)
	{
	  ...
	  {
	    #pragma acc kernels copyin(N, A[0:N]) async(omp_get_num_threads())
		  for(int j=0; j<N; j++)
		  {
		    A[j] = A[j] + 1;
		  }
	  ...
	  {
	    #pragma acc kernels copyin(N,B[0:N]) async(omp_get_num_threads())
		  for(int k=0; k<N; k++)
		  {
		    B[k] = B[k] + 1;
		  }
...
int main()
{
	...
	static const char * logFileName = "test_acc_kernels_async.log";	/* name of the logfile */
	...
	printf("##############################################\n");
	printf("Testing acc_kernels_async\n\n");

	fprintf(logFile,"######## OpenACC Validation Suite V %s ######\n", ACCTS_VERSION );
	fprintf(logFile,"## Repetitions: %3d                       ####\n",REPETITIONS);
	fprintf(logFile,"## Loop Count : %6d                    ####\n",LOOPCOUNT);
	fprintf(logFile,"##############################################\n");
	fprintf(logFile,"Testing acc_kernels_async\n\n");

	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of test_acc_kernels_async out of %d\n\n",i+1,REPETITIONS);
		if(test_acc_kernels_async(logFile)){
